function [digital, nl_digital] = capture_with_additive_binning(scene, ...
                                                    sensor, binning, gain)
...
for cc = 1: 3
    binnedVoltage(:,:,cc) = binVoltage(voltage(:,:,cc), binning, 'additive'); 
end
amplifiedVolt = gain * binnedVoltage;
digital = AD_conversion(amplifiedVolt, sensor.maxVolt, sensor.ADC_std, ...
    sensor.bit, sensor.blackLevel);
...
digital = digital / (binning ^ 2) / gain;
digital = double(digital);
